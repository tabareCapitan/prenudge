...
Description:  OLS conditional estimates using binary info preferences measure

Created: 20220301 | Last modified: 20220729
*******************************************************************************/
...
gsreg infoSelected $COVS, fixvar(group) cmdoptions(robust) nocount             ///
                           replace   resultsdta("$RUTA/temp/data/allRegs_TS")


// Load data of all estimates

use "$RUTA/temp/data/allRegs_TSS.dta", clear
...
twoway  (scatter group_c group_p1T,  msymbol(o) msize(small)  mcolor(gs10) )
        ...
        legend(off)
        yscale(alt,)
        ylabel(0.05(0.01)0.11, ang(horizontal) )
        xscale(alt)
...
graph export "$RUTA/results/figures/oa_treatmentEffect_TS.png",
...
// for now let's use these covariates that "make sense"
global COVS "hungryLevel female foodSelfControl BMI weightGoals_3 dieting knowsCalorieNeeds"

// Run all possible regressions

gsreg infoSelected $COVS , fixvar(group) cmdoptions(robust) nocount             ///
                           replace   resultsdta("$RUTA/temp/data/allRegs_TF")


// Load data of all estimates

use "$RUTA/temp/data/allRegs_TF.dta", clear
...
twoway  (scatter group_c group_p1T,  msymbol(o) msize(small)  mcolor(gs10) )
        ...
        legend(off)
        yscale(alt,)
        ylabel(0.04(0.01)0.07, ang(horizontal) )
        xscale(alt)
        xlabel(0.01 "0.01" 0.05 "0.05" 0.1 "0.1", grid gextend)
        xtitle("pvalue")
        ytitle("Treatment effect", orientation(rvertical))
...
graph export "$RUTA/results/figures/oa_treatmentEffect_TF.png",
                                    replace width(11000) height(8000);
